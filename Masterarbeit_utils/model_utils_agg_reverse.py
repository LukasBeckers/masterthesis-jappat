...
def get_tokenizer(dump_dir):
    ...
    if os.path.isfile(f'{dump_dir}/agg_label_embedding_no_single.pk'):
        with open(f'{dump_dir}/agg_label_embedding_no_single.pk', 'rb') as f:
            dataset_emb = pk.load(f)
            f_term_dict = dataset_emb.dict
    else:
        with open(f'{dump_dir}/agg_f_terms_in_ds_dir.pk', 'rb') as f:
            f_term_dict = pk.load(f)
        
    # Loading a dict, which contains all uniqe f-terms with crawled definitions
    with open(f'{dump_dir}/agg_f_term_dict.pk', 'rb') as f:
...
def modify_embeddings(model: OPTForCausalLM, n_f_terms: int, device: str) -> OPTForCausalLM:
    ...
    print('New Embedding', new_embeddings, 'Number of F-Terms', n_f_terms)
