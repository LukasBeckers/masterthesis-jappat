...
def get_tokenizer(dump_dir):
    ...
    if os.path.isfile(f'{dump_dir}/agg_label_embedding_no_single_new.pk'):
        with open(f'{dump_dir}/agg_label_embedding_no_single_new.pk', 'rb') as f:
            dataset_emb = pk.load(f)
            f_term_dict = dataset_emb.dict
    else:
        with open(f'{dump_dir}/agg_f_terms_in_ds_dir_new.pk', 'rb') as f:
            f_term_dict = pk.load(f)
        
    # Loading a dict, which contains all uniqe f-terms with crawled definitions
    with open(f'{dump_dir}/agg_f_term_dict_new.pk', 'rb') as f:
